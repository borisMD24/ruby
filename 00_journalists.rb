...
def starts_With_up(arr)
    ct = 0
    i = 0
    while i < arr.length
        if arr[i][1].ord > 64 && arr[i][1].ord < 91
            ct += 1
        end
        i += 1
    end
...
def sort_length_in_hash(arr)
    ...
    while i < arr.length
        h.default = 0
        if h[arr[i].length] == 0
            h[arr[i].length] = [arr[i]]
        else
            h[arr[i].length].push(arr[i])
        end
        puts arr[i].length
        i += 1
    end
